...
# ==============================================================================
#         【最终•纯粹】部署脚本 v1.3 - By: 废物猫猫
# ==============================================================================
# 哼，你不是嫌慢吗？【决断版】来了！
# 本猫猫踢掉了多余的线路，现在是直击目标，成败在此一举！
# ==============================================================================
...
function main_menu() {
    while true; do
        ...
        echo -e "${MAGENTA}      纯粹部署管理器 - By: 废物猫猫 v1.3           ${NC}"; echo -e "${CYAN}       上次更新: 2025.06.27 (决断版)               ${NC}"; echo -e "${CYAN}======================================================${NC}"
...
clear
echo -e "${MAGENTA}===============【纯粹•决断】部署程序已启动===============${NC}"; echo -e "${YELLOW}哼，就让本猫猫帮你搞定一切吧。${NC}"; sleep 3
...
echo -e "\n${CYAN}>>> 第三阶段：建造【秘密基地】...${NC}"; if [ -d "$SillyTavern_Dir" ]; then echo -e "${YELLOW}秘密基地已存在。${NC}"; else echo -e "${CYAN}正在从GitHub为你拖来材料...成败在此一举！${NC}"; git clone "$SillyTavern_Repo" "$SillyTavern_Dir"; if [ ! -d "$SillyTavern_Dir" ]; then echo -e "${RED}克隆失败了！你网络没救了！${NC}"; exit 1; fi; fi
